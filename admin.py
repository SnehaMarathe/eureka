# app.py — EurekaCheck Diagnostic (Trace-only Loose Ground + DM1 + Presence)
# - TP/BAM reassembly, DM1 with corrected lamp parsing, Clean DM1 Table
# - Firebase upload (ECU presence, DTCs)
# - NEW: Loose Ground Detector (trace-only, no PDF parsing)
# - Optional: archive uploaded .trc to Firebase Storage (if configured)
...
def archive_trace_to_firebase(vehicle_name: str, uploaded_filename: str, file_bytes: bytes):
    """
    Try Storage first (if configured), else log trimmed base64 to Firestore.
    """
    if (bucket is not None) and STORAGE_AVAILABLE:
        try:
            ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            safe_v = re.sub(r"[^A-Za-z0-9_\-]+", "_", vehicle_name.strip() or "vehicle")
            safe_f = re.sub(r"[^A-Za-z0-9_\-\.]+", "_", uploaded_filename or "trace.trc")
            blob_path = f"traces/{safe_v}/{ts}_{safe_f}"
            blob = bucket.blob(blob_path)
            blob.upload_from_string(file_bytes, content_type="text/plain")
            # You can make it public if your policy allows:
            # blob.make_public()
            ...
            if db is not None:
                db.collection("trace_archives").add(meta)
            st.success("☁️ Trace archived to Firebase Storage.")
            return
        except Exception as e:
            st.warning(f"⚠️ Storage upload failed: {e} — trying Firestore fallback")

    # Firestore fallback (stores metadata + small preview only)
    if db is not None:
        try:
            preview_len = min(len(file_bytes), 64 * 1024)  # 64KB preview
            preview_b64 = base64.b64encode(file_bytes[:preview_len]).decode("ascii")
            payload = {
                "vehicle": vehicle_name,
                "file": uploaded_filename,
                "size_bytes": len(file_bytes),
                "preview_b64_len": preview_len,
                "preview_b64": preview_b64,
                "user_info": _current_user_info(),
                "timestamp": datetime.now().isoformat(),
                "note": "Full file not stored (Storage unavailable)."
            }
            db.collection("trace_archives").add(payload)
            st.success("☁️ Trace archived to Firestore (preview only).")
        except Exception as e:
            st.warning(f"⚠️ Firestore archival failed: {e}")
...
# -------------------------
# ECU connector map & drawing map (kept for service hints)
# -------------------------
ecu_connector_map = {
...
# Power/Ground topology (static defaults)
ground_map_default = {
    "Engine ECU": "G101",
    "ABS ECU": "G201",
    "Telematics": "G202",
...
# -------------------------
# Utility: PDF report generation (presence PDF)
# -------------------------
...
def infer_root_causes(df: pd.DataFrame):
    ...
    for _, row in df[df["Status"] == "❌ MISSING"].iterrows():
        ...
        harness = ecu_connector_map.get(row["            harness = ecu_connector_map.get(row["ECU"], {}).get("harness", "-")
...
- 🔎 **Drawing hint**: `{drawing or '—'}`

### 🛠️ Suggested Checks:
1. Verify voltage from **{fuse}** to **{connector}** (key ON).
2. Inspect CAN lines at `{connector}`; check for corrosion/moisture.
3. Inspect `{harness}` bends & clips for chafing or open.
"""
...
# -------------------------
# LOOSE GROUND DETECTOR (trace-only)
# -------------------------
FMI_SHORT_TO_BATT = {3}
FMI_SHORT_TO_GND  = {4}
FMI_OPEN_CIRCUIT  = {5}
...
def extract_battery_voltage_stats(df_can: pd.DataFrame):
    """SPN 168 voltage from PGN 0xFEF7 (Engine #1)."""
    if df_can.empty:
        return None
    volt = []
    for _, r in df_can.iterrows():
        ...
        if pgn == 0xFEF7 and len(data) >= 2:
            ...
            if raw not in (0xFF, 0xFE, 0xFFFF):
                volt.append(raw * 0.05)  # 0.05 V/bit
    if not volt:
        return None
    return {
        "min_v": round(min(volt), 2),
        "max_v": round(max(volt), 2),
        "avg_v": round(sum(volt)/len(volt), 2),
        "n": len(volt)
    }

def detect_addr_flapping(df_can: pd.DataFrame, gap_seconds: float = 3.0, min_msgs: int = 100, min_long_gaps: int = 5):
    """Intermittent broadcast (power/ground/ignition issues)."""
...
def loose_ground_assessment(df_presence: pd.DataFrame, raw_dtcs_df: pd.DataFrame, df_can: pd.DataFrame):
    """
    Returns list of plain-English service actions focused on ground/loose power.
    """
    actions = []
    # 1) Presence clustering by Fuse/Connector/Harness/Ground
    presence = df_presence.copy()
    presence["Harness"] = presence["ECU"].map(lambda e: ecu_connector_map.get(e, {}).get("harness", "-"))
    presence["Ground"]  = presence["ECU"].map(lambda e: ground_map_default.get(e, "—"))
    missing = presence[presence["Status"] == "❌ MISSING"].copy()
    def cluster(by):
        if missing.empty: return
        for key, g in missing.groupby(by):
            if key in (None, "-", "", "—"): continue
            affected = g["ECU"].tolist()
            total = sum(1 for row in presence.itertuples() if getattr(row, by, None) == key)
            share = (len(affected) / max(1, total))
            if share >= 0.5 and len(affected) >= 2:
                label = by
                actions.append({
                    "priority": 1,
                    "title": f"Multiple ECUs missing on shared {label}: {key}",
                    "what_to_do": [
                        f"Inspect ground/power continuity at {label} `{key}`.",
                        "Wiggle-test harness while monitoring CAN; re-check presence.",
                        "Measure voltage at affected ECU connectors (key ON)."
                    ],
                    "affected_ecus": affected
                })
    for by in ("Fuse", "Connector", "Harness", "Ground"):
        cluster(by)

    # 2) DTC patterns (FMI 4/5/6) -> wiring/ground
    if raw_dtcs_df is not None and not raw_dtcs_df.empty:
        by_fmi = defaultdict(list)
        for _, r in raw_dtcs_df.iterrows():
            fmi = r.get("FMI")
            if pd.notna(fmi):
                by_fmi[int(fmi)].append(str(r.get("Source Address")))
        if 4 in by_fmi or 5 in by_fmi or 6 in by_fmi:
            actions.append({
                "priority": 2,
                "title": "DTCs indicate wiring/ground faults",
                "what_to_do": [
                    "Check pin tension & corrosion on ECU/actuator connectors.",
                    "Perform continuity test (sensor ↔ ECU), wiggle harness.",
                    "Clean & retorque chassis grounds (especially shared ones)."
                ],
                "evidence": { "FMIs": {k: len(v) for k,v in by_fmi.items() if k in (4,5,6)} }
            })
    ...
    if flap:
        ...
        for sa_int, meta in flap.items():
            ...
            pretty.append(f"{ecu} (gaps={meta['gaps']})")
        actions.append({
            "priority": 0,
            "title": "Intermittent ECU broadcasts (possible loose power/ground)",
            "what_to_do": [
                "Check battery clamps/ground straps; perform voltage-drop test while loads ON.",
                "Inspect main power/ground splices feeding flapping ECUs.",
                "Backprobe ECU B+ and GND for dropouts during wiggle test."
            ],
            "evidence": pretty
        })
    ...
    if vstats and vstats["min_v"] <= 10.5:
        actions.append({
            "priority": 0,
            "title": f"System voltage low (min {vstats['min_v']} V)",
            "what_to_do": [
                "Charge/replace battery if needed.",
                "Check alternator output and ground straps.",
                "Re-test after stabilizing supply voltage."
            ],
            "evidence": vstats
        })

    actions.sort(key=lambda x: (x["priority"], x.get("title","")))
    return actions
...
# -------------------------
# CAN Data Input (+ archive to Firebase)
# -------------------------
...
if uploaded_file is not None:
    ...
    # Archive to Firebase (Storage if available, else Firestore preview)
    try:
        archive_trace_to_firebase(vehicle_name or "unknown", uploaded_name, uploaded_bytes)
    except Exception as e:
        st.warning(f"⚠️ Archival skipped: {e}")
    # Parse
...
# Root Cause (presence clustering)
st.subheader("🧠 Root Cause (Presence Clusters)")
...
# Presence PDF (download)
pdf_buf = generate_pdf_buffer(report, vehicle_name)
st.download_button("⬇️ Download Presence Report (PDF)", pdf_buf, f"{vehicle_name}_presence.pdf", "application/pdf")

# Detailed ECU diagnostics for missing ECUs
st.subheader("🔧 Detailed ECU Diagnostics (for MISSING ECUs)")
...
# -------------------------
# Loose Ground Detector — Service Actions
# -------------------------
st.markdown("---")
st.subheader("🧲 Loose Ground / Power Risk — Service Actions (Trace-only)")

service_actions = loose_ground_assessment(df_report, raw_dtcs, df_can)
if not service_actions:
    st.info("No ground/power risk patterns detected from this trace.")
else:
    for act in service_actions:
        ev = act.get("evidence")
        affected = act.get("affected_ecus")
        bullets = "".join([f"<li>{s}</li>" for s in act["what_to_do"]])
        extra = ""
        if affected:
            extra += f"<br><i>Affected ECUs:</i> {', '.join(affected)}"
        if ev:
            extra += f"<br><i>Evidence:</i> {ev}"
        st.markdown(
